...
class CreateShortTerm_0: UIViewController {
    
    var goalType : Int?

    let greetingView : GreetingViewSubclass = {
        let view = GreetingViewSubclass()
        view.titleLabel.text = "Short Term Goal"
        view.subLabel_0.text = "Aim to achieve this by tomorrow.."
    ...
    var goalName : UITextField = {
        ...
        text.placeholder = "Goal Name"
        text.textAlignment = .center
        text.font = UIFont(name: "Helvetica Neue", size: 35)
        text.layer.cornerRadius = 5
        text.layer.borderColor = UIColor.gray.cgColor
        text.addTarget(self, action: #selector(moveText), for: .editingDidEnd)
        text.addTarget(self, action: #selector(moveText), for: .editingDidBegin)
...
//        self.title = "Goal Name"
    ...
    func setupConstraints() {
        ...
        goalName.center.x = w * 0.1
        goalName.center.y = h / 3
        goalName.frame.size = CGSize(width: self.view.frame.width * 0.8, height: 0.4 * (self.view.frame.width * 0.8))
    ...
    @objc func moveText(sender: UITextField) {
        if sender.isEditing {
            // move up
            UIView.animate(withDuration: 0.5) {
                sender.center.y = sender.center.y - 20
            }
        } else {
            // move back down
            UIView.animate(withDuration: 0.5) {

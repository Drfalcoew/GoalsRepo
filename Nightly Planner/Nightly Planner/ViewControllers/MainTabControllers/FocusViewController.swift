...
class FocusViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    let helpButton : UIButton = {
        ...
        btn.imageView?.tintColor = UIColor(r: 221, g: 221, b: 221)
        btn.translatesAutoresizingMaskIntoConstraints = false
        btn.layer.masksToBounds = true
        btn.addTarget(self, action: #selector(handleHelp), for: .touchUpInside)
        
        btn.imageView?.tintImageColor(color: .white)
        return btn
    }()
